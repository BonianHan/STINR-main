...
class DeconvNet(nn.Module):
    ...
    def __init__(self, 
        ...
        self.deconv_beta_layer = DenseLayer(hidden_dims[2], n_celltypes, 
                                                          zero_init=True)
    ...
    def forward(self, 
        ...
        Z, mid_fea = self.encoder(self.coord)
        ...
        # self.fea_loss = 2*torch.norm(node_feats-self.node_feats_recon, 2)
        # Total loss
    ...
    def evaluate(self, adj_matrix, coord, node_feats, slice_label):
        coord_scaled = coord / 100
        slice_label_emb = self.slice_emb(slice_label)
        Z, _ = self.encoder(coord_scaled)
        beta, alpha = self.deconvolutioner(Z, slice_label_emb)
        
        return Z, beta, alpha, self.gamma
            
    def encoder(self, coord):
        self.mid_fea = self.encoder_layer0(coord)
