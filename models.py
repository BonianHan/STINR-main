...
class Model():
    def __init__(self, adata_st, adata_basis, 
                 annotations_dir='DLPFC_annotations',
                 hidden_dims=[512, 128],
                 n_heads=1,
                 slice_emb_dim=16,
                 coef_fe=0.1,
    ...
    def _load_ground_truth(self):
        ground_truth_dict = {}
        for slice_id in self.adata_st.obs['slice_id'].unique():
            slice_id_str = str(int(slice_id))
            truth_file = os.path.join(self.annotations_dir, f'{slice_id_str}_truth.txt')

            if os.path.exists(truth_file):
                ann_df = pd.read_csv(truth_file, sep='\t', header=None, index_col=0)
                for barcode, gt in ann_df.iloc[:, 0].items():
                    ground_truth_dict[barcode] = gt

        if ground_truth_dict:
            barcodes = self.adata_st.obs.index.str.replace(r'-slice\d+$', '', regex=True)
            self.adata_st.obs['Ground Truth'] = barcodes.map(ground_truth_dict)

    def train(self, report_loss=True, step_interval=14001):
        self.net.train()
        ...
        for step in tqdm(range(self.training_steps)):
            ...
            if step % 2500 == 0:
                ...
                adata_st_list_raw.append(adata_st_list_raw3)
                            # 将打乱后的坐标从 self.adata_st 复制到 adata_st_list_raw
                n_spots = 0
                for i, adata_slice in enumerate(adata_st_list_raw):
                    n_spots_slice = adata_slice.shape[0]
                    adata_slice.obsm['3D_coor'] = self.adata_st.obsm['3D_coor'][n_spots:n_spots+n_spots_slice].copy()
                    adata_slice.obsm['spatial'] = adata_slice.obsm['3D_coor'][:, :2]  # ← 新增这行!
                ...
                for i in range(len(result)):
                    ...
                    sc.pl.spatial(result[i], img_key="hires", 
                              color=["GM","Ground Truth"], 
                              title=['Slice'+str(slice_idx[i])+' results ARI='+str(round(ari,3)), 
                                    'Ground Truth'],
                              spot_size=100)
